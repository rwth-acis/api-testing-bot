...
public class CodexTestGen {
     ...
     */
    private String insert(String code, String instruction) throws CodexAPI.CodexAPIException {
        // split code into two parts - before and after [insert]
        String input = code.split("\\[insert]")[0];
        String suffix = code.split("\\[insert]")[1];

        // call OpenAI API
        JSONArray choices = new CodexAPI(codexAPIToken,openAIModel).insert(input, instruction);
